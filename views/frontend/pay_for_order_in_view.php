...
$plugin_db = json_encode($getallplans);
...
if ( $order->get_status() == "pending" ) {
    printf(
        '<a class="woocommerce-button wp-element-button button pay" id="pay_btn">Pay for this order</a>',
    );
}
...
foreach ( $order->get_items() as $item_id => $item ) {
    ...
    var item_for_pay_btn = <?php print $item ?>;
    var plugin_db = <?php print $plugin_db ?>;
    var order_pay_for_order_btn = <?php print $order ?>;

    var customer_email = order_pay_for_order_btn.billing.email;
    if (item_for_pay_btn.meta_data.length>0) {
        if (item_for_pay_btn.meta_data[0]?.key === "Subscription") {
            var billing_cycle = item_for_pay_btn.meta_data[0].value;
            var split = billing_cycle.split(" ");
            var product_object = {
                name: item_for_pay_btn.name,
                amount: JSON.parse(item_for_pay_btn.total),
                currency: order_pay_for_order_btn.currency,
                plan_id: item_for_pay_btn.product_id,
                billing_cycle: split[1],
                frequency: split[0],
                isOneTypePayment: false
            }
            chargely_send_arr.push(product_object);
        }
    }
    if (item_for_pay_btn.meta_data.length == 0) {
        var product_object = {
    ...
    var data = {
        plan_object: chargely_send_arr,
        shipping_amount: JSON.parse(order_pay_for_order_btn.shipping_total),
        order_id: item_for_pay_btn.order_id,
        customer_email : customer_email,
        billing_details: order_pay_for_order_btn.billing,
        shipping_details: order_pay_for_order_btn.shipping,
        ...
        redirect_url: `${plugin_db[0].url_endpoint}/my-account/view-order/${item_for_pay_btn.order_id}/`   
    }
                    ...
                    if (parseInt(element.order_id) === parseInt(order_id_check)) {
                        ...
                        if (api_url_order && consumer_key_order && oauth_signature_Order && timestampOrder && nonceOrder && oauthSignatureMethodOrder && oauthVersionorder) {
                            ...
                            update_order_request.onreadystatechange=()=>{
                                ...
                                function myGreeting() {
                                    window.location.href = `${auth[0]?.url_endpoint}/checkout/order-received/${order_id_res}/?key=${order_key_res}`;
                                    AmagiLoader.hide();
                                }
                            }
